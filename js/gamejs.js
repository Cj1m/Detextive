...
window.onLoad = (function($) {
  ...
  $.fn.writeText = function(content) {
    ...
    typeTime = 5; /*Very fast for immediate results, good reading speed is 45*/

    setInterval(function() {
      if(current < contentArray.length) {
        elem.text(elem.text() + contentArray[current++]);
        mainCon = document.getElementById("paraText");
...
function printText() {
  $("#inputText").keypress(function(event){
    if(event.keyCode == 13){
        printText = document.getElementById.write("inputText");
    }
});
}
